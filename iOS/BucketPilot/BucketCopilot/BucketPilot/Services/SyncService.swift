...
final class SyncService {
    ...
    init(
        ...
        encoder.dateEncodingStrategy = .custom { date, encoder in
            ...
            let string = SyncService.makeISO8601WithFractional().string(from: date)
        ...
        decoder.dateDecodingStrategy = .custom { decoder in
            ...
            if let date = SyncService.makeISO8601WithFractional().date(from: string) {
                return date
            }
            if let date = SyncService.makeISO8601().date(from: string) {
                return date
            }
    ...
    private func pullEvents() async throws -> SyncPullResponse {
        ...
        if let lastTimestamp = state.lastSyncTimestamp {
            let timestamp = SyncService.makeISO8601WithFractional().string(from: lastTimestamp)
    ...
    private func allocationPayload(for event: AllocationEvent) -> [String: JSONValue] {
        var payload: [String: JSONValue] = [
            ...
            "timestamp": .string(SyncService.makeISO8601WithFractional().string(from: event.timestamp)),
    ...
    private nonisolated static func makeISO8601WithFractional() -> ISO8601DateFormatter {
        let formatter = ISO8601DateFormatter()
        formatter.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
        return formatter
    }
    
    private nonisolated static func makeISO8601() -> ISO8601DateFormatter {
        let formatter = ISO8601DateFormatter()
